    ...
    open fun updateNote(date: Date,
        ...
        executor.execute {
            try {
                ...
                result.postValue(AsyncLoad.error(message = "error while trying to update note"))
            }
        }
    }
    ...
    open fun loadNote(date: Date, result: MutableLiveData<AsyncLoad<Note>>) {
        ...
        executor.execute {
            try {
                ...
                val note: Note? = Converter.noteItemToNote(noteItem)
                result.postValue(AsyncLoad.success(note))
            } catch (e: Exception) {
                logWarn(e) { "Error while loading note" }
                result.postValue(AsyncLoad.error(message = "Error while loading note"))
            }
        }
    }
